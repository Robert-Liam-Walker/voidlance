...
export class FxLayer {
  ...
  constructor(private scene: THREE.Scene, private camera: THREE.Camera, private canvas: HTMLCanvasElement, private overlay: HTMLElement) {
    ...
    for (let i = 0; i < 256; i++) {
      ...
      mesh.visible = false;
      scene.add(mesh);
      this.freeParticles.push({ mesh, vx: 0, vy: 0, vz: 0, life: 0, max: 1 });
    }
  ...
  handle(ev: FxEvent): void {
    switch (ev.type) {
      ...
      case 'shake': this.shakeMag = Math.max(this.shakeMag, ev.intensity * 60); this.shakeLeft = Math.max(this.shakeLeft, ev.durMs); break;
  ...
  private burst(x: number, y: number, color: number, count: number): void {
    ...
    for (let i = 0; i < count; i++) {
      ...
      p.vy = Math.sin(el) * sp * 0.6 + 2;
      p.max = 0.24 + Math.random() * 0.24;
      p.life = p.max;
  ...
  private ring(x: number, y: number, color: number, radius: number): void {
    ...
    mesh.position.set(worldX(x), HOVER_Y - 1.5, worldZ(y));
    mesh.scale.setScalar(0.05);
    this.scene.add(mesh);
    this.rings.push({ mesh, life: 0.48, max: 0.48, target: 1 });
  }
  ...
  private beam(x: number, y0: number, y1: number, color: number, width: number, durMs: number): void {
    ...
    const mesh = new THREE.Mesh(
      ...
      new THREE.MeshBasicMaterial({ color, transparent: true, opacity: width > 10 ? 0.8 : 0.3, blending: THREE.AdditiveBlending, depthWrite: false })
    );
    mesh.position.set(worldX(x), HOVER_Y, worldZ((y0 + y1) / 2));
    this.scene.add(mesh);
    this.beams.push({ mesh, life: durMs / 1000, max: durMs / 1000 });
  }
  ...
  update(dtMs: number): void {
    ...
    for (let i = this.rings.length - 1; i >= 0; i--) {
      ...
      r.mesh.scale.setScalar(0.05 + k * (r.target - 0.05) + k * 20);
    ...
    for (let i = this.beams.length - 1; i >= 0; i--) {
      ...
      b.life -= dt;
      (b.mesh.material as THREE.MeshBasicMaterial).opacity *= Math.max(0, b.life / b.max);
      if (b.life <= 0) {
  ...
  /** Camera positional jitter to add this frame (decays with the shake timer). */
  shakeOffset(): THREE.Vector3 {
    if (this.shakeMag <= 0) return ZERO;
